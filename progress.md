...
提交阶段 11 收尾修改，并启动阶段 12“发布准备”。

**本轮提交：**
- 提交哈希：`43728f6`
- 提交信息：`docs(zh): complete stage 11 proofreading`

**阶段 12 启动结果：**
- 核对 Git 远程配置，确认 `origin` 已指向 `https://github.com/brand6/Claude-Code-Game-Studios-zh.git`
- 因此，“把 zh/ 目录连接到远程仓库” 这一步在仓库层面已具备条件，无需额外切换远程
- 将根目录 `README.md` 切换为中文入口文档，并补充“本仓库为原仓库中文翻译 / 本地化发布版本”的说明
- `task_plan.md`：将阶段 12 状态改为 `in_progress`，记录当前已完成与待执行的发布准备项
...
**验证：**
- `git status --short`：提交后仅剩 `.obsidian/workspace.json` 为未跟踪到本次提交的本地状态变更
