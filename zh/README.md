...
| `validate-assets.sh` | PostToolUse（写入/编辑） | 验证命名规范和 JSON 结构——文件不在 `assets/` 下时提前退出 |
...
| `validate-skill-change.sh` | PostToolUse（写入/编辑） | 在 `.claude/skills/` 下发生变更后，建议运行 `/skill-test` |
